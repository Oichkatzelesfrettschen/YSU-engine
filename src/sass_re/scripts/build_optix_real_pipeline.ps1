<#
.SYNOPSIS
  Build the OptiX real pipeline device PTX and host runner.
...
.NOTES
  Equivalent of build_optix_real_pipeline.sh.
  Requires: nvcc, OptiX headers under /usr/include/optix or $env:OPTIX_INCLUDE
#>
...
& $Nvcc -arch=sm_89 $StdFlag -lineinfo --ptx `
    "-I/usr/include" "-I$OptixInc" `
    (Join-Path $RunnerDir "optix_real_pipeline_device.cu") `
    -o $PtxOut
if ($LASTEXITCODE -ne 0) { throw "Device PTX compilation failed" }

& $Nvcc -arch=sm_89 $StdFlag -lineinfo `
    "-I/usr/include" "-I$OptixInc" `
    (Join-Path $RunnerDir "optix_real_pipeline_runner.cu") `
